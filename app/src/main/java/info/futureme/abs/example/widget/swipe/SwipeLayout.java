package info.futureme.abs.example.widget.swipe;
...
public class SwipeLayout extends FrameLayout {
    ...
    double initX = -1, initY = -1;

    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_UP
                || ev.getAction() == MotionEvent.ACTION_CANCEL) {
            if (Math.abs(ev.getRawX() - initX) > 10 || Math.abs(ev.getRawY() - initY) > 10) {
                click = false;
                if ((Math.abs(ev.getRawX() - initX) > Math.abs(ev.getRawY() - initY)) && !isSwipeEnabled() && ev.getRawX() - initX < 0) {
                    Toast.makeText(getContext(), getContext().getString(R.string.wait_for_closing), Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void performAdapterViewItemClick() {
        if (getOpenStatus() != Status.Close || !click) return;
        ViewParent t = getParent();
        if (t instanceof AdapterView) {
    ...
    public Status getOpenStatus() {
        ...
        if (surfaceLeft == (getPaddingLeft() - mDragDistance + WindowUtils.getWindowWidth() - WindowUtils.dp2px(80)) || surfaceLeft == (getPaddingLeft() + mDragDistance)
    ...
    private Rect computeSurfaceLayoutArea(boolean open) {
        ...
        if (open) {
            if (mCurrentDragEdge == DragEdge.Left)
                ...
                l = getPaddingLeft() - mDragDistance + (WindowUtils.getWindowWidth() - WindowUtils.dp2px(80));
    ...
    private Rect computeBottomLayoutAreaViaSurface(ShowMode mode, Rect surfaceArea) {
        Rect rect = computeSurfaceLayoutArea(false);//urfaceArea;
